...
public class XPathFunctions {
    ...
    private static String fullXpathWithPosition(Attribute attribute) {
        String path = "/" + attribute.getParent().getQualifiedName() + "["
                + getPosition(attribute.getParent()) + "]" + "/@"
                + attribute.getQualifiedName();
        Element element = attribute.getParent();
        String parent = "";
        if (element.getParentElement() != null && !element.isRootElement()) {
            if (!element.getParentElement().getQualifiedName()
                    .equals("as:Structure")
                    && !element.getParentElement().getQualifiedName()
                    .equals("as:Temp")) {
                parent = fullXpathWithPosition(element.getParentElement());
    ...
    private static String xpathParentAndAll(Element elem) {
        ...
        if (!(elem.getParentElement().getName().equals("Structure") && elem
                .getParentElement().getNamespace()
    ...
    private static String xpathParentAndAll(Attribute attr) {
        ...
        if (!(attr.getParent().getName().equals("Structure") && attr
                .getParent().getNamespace().equals(CAMConstants.CAMNamespace))) {
            if (!path.startsWith("/"))
    ...
    private static int getPosition(Element element) {
        if (element.getParentElement() != null) {
            ...
            for (Element elem : list) {
                if (elem.getName().equals(element.getName())
                        && elem.getNamespace().equals(element.getNamespace()))
                    count++;
                if (elem.equals(element)) {
                    return count;
                }
    ...
    @SuppressWarnings("unchecked")
    private static void drillDown(Object original, Element element,
                                  String path, List<String> paths, List<Element> nodes) {
