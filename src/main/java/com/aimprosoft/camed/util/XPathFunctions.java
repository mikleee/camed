...
public class XPathFunctions {
    ...
    private static String xpathToParent(Element element, int depth) {
        StringBuilder result = new StringBuilder("/").append(element.getQualifiedName());
        ...
        while (position < depth) {
            ...
            if (current != null) {
                result.insert(0, current.getQualifiedName()).insert(0, "/");
            } else {
                break;
            }
        }
